...
<section class="relative overflow-hidden bg-islamic-pattern text-white py-20 lg:py-28 border-b-4 border-gold-500/80">
    ...
    <div class="absolute -top-24 left-1/2 -translate-x-1/2 w-[750px] h-[750px] bg-gradient-to-b from-gold-500/15 via-rose-500/10 to-transparent rounded-full blur-3xl pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-7 space-y-6 text-center lg:text-left" data-aos="fade-right">
                <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-rose-950/80 border border-gold-500/30 text-xs text-gold-400 font-semibold uppercase tracking-widest backdrop-blur shadow-inner">
                ...
                <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.18] text-white">
                    ...
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-rose-200 via-gold-300 to-amber-400 font-serif">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4 pt-4 justify-center lg:justify-start">
                    ...
                    <a href="#programs-section" class="w-full sm:w-auto px-8 py-4 rounded-2xl bg-rose-950/60 hover:bg-rose-900/80 border border-emerald-500/30 text-white font-semibold text-sm backdrop-blur hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                        <i data-lucide="book-open" class="w-4 h-4 text-gold-400"></i> Katalog Program & Silabus
                    </a>
                </div>

                <!-- 3 Pillars Mini Info -->
                <div class="pt-8 grid grid-cols-3 gap-3 sm:gap-4 border-t border-emerald-800/60 text-left">
                    <div class="p-3 rounded-2xl bg-rose-950/40 border border-emerald-800/40 hover:border-gold-500/40 transition-colors">
            ...
            <!-- Right Interactive 4-Pillars Explorer -->
            <div class="lg:col-span-5 relative" data-aos="fade-left" x-data="{ 
                activePillar: 1,
                pillars: [
                    { id: 1, arabic: '١', name: 'READ', title: 'Qira\'ah Dasar', desc: 'Mengenal huruf hijaiyah, harakat, sukun, tasydid hingga membaca kata secara mandiri.', tag: 'From Zero', goal: 'Mampu Membaca Mandiri' },
                    { id: 2, arabic: '٢', name: 'IMPROVE', title: 'Tahsin & Tajwid', desc: 'Menyempurnakan 5 tempat makhraj huruf, sifatul huruf, dan hukum tartil mutawatir.', tag: 'Correct Recitation', goal: 'Makhraj Presisi & Tartil' },
                    { id: 3, arabic: '٣', name: 'UNDERSTAND', title: 'Tadabbur Ayat', desc: 'Menyelami mufradat (kosakata Al-Qur\'an), asbabun nuzul, dan hikmah pesan surah.', tag: 'Deep Connection', goal: 'Paham Makna & Refleksi' },
                    { id: 4, arabic: '٤', name: 'LIVE', title: '\'Amal & Fardhu \'Ain', desc: 'Mempraktikkan adab Qur\'ani, fiqh thaharah & shalat, serta akhlaqul karimah.', tag: 'Practice Faith', goal: 'Amalan Nyata Sehari-hari' }
                ]
            }">
                <div class="bg-gradient-to-b from-rose-950/95 to-slate-950/95 border-2 border-gold-500/30 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-2xl text-white space-y-5 relative overflow-hidden">
                    ...
                    <div class="flex items-center justify-between border-b border-emerald-800/60 pb-4">
                        ...
                        <span class="text-[10px] px-2.5 py-1 rounded-full bg-rose-500/20 text-emerald-300 font-semibold border border-emerald-500/30">Step-by-step</span>
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        <template x-for="p in pillars" :key="p.id">
                            <button @click="activePillar = p.id" 
                                    :class="activePillar === p.id ? 'bg-gold-500 text-slate-950 font-bold border-gold-400 shadow-md scale-105' : 'bg-rose-900/40 text-emerald-200 border-emerald-700/40 hover:bg-rose-800/50'"
                                    class="py-2 px-1 rounded-xl text-center border text-xs transition-all flex flex-col items-center cursor-pointer">
                    ...
                    <div class="p-4 rounded-2xl bg-rose-900/30 border border-gold-500/30 min-h-[140px] flex flex-col justify-between transition-all">
                        <template x-for="p in pillars" :key="p.id">
                            <div x-show="activePillar === p.id" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-2">
                                ...
                                <div class="pt-3 flex items-center justify-between text-[11px] text-emerald-300 border-t border-emerald-800/40">
                                    <span>Target: <strong class="text-white" x-text="p.goal"></strong></span>
                                    <span class="text-gold-400 font-semibold">Talaqqi Privat &rarr;</span>
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <div class="pt-1">
                        <a href="{{ route('register') }}" class="block w-full py-3.5 rounded-2xl bg-gradient-to-r from-rose-600 to-teal-700 hover:from-rose-500 hover:to-teal-600 text-white font-bold text-xs shadow-lg transition-all border border-emerald-400/30 text-center hover:scale-[1.02]">
...
<!-- 4 Core Programs Section with Instant Filter -->
<section id="programs-section" class="py-24 bg-islamic-subtle relative" x-data="{ 
    search: ''
}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-3xl mx-auto mb-10 space-y-3" data-aos="fade-up">
            <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-rose-100/80 border border-primary-300/50 text-xs font-bold text-primary-900 uppercase tracking-widest">
            ...
            <!-- Search Bar -->
            <div class="pt-4 flex items-center justify-center max-w-md mx-auto">
                <div class="relative w-full">
                    <input type="text" x-model="search" placeholder="Cari topik modul (contoh: makhraj, tajwid, shalat)..." class="w-full pl-10 pr-4 py-2.5 rounded-2xl border-2 border-emerald-900/10 focus:border-gold-500 bg-white text-xs shadow-sm outline-none transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            @foreach($programs as $program)
                <div x-show="!search || '{{ addslashes(strtolower($program->name . ' ' . $program->tagline . ' ' . $program->description)) }}'.includes(search.toLowerCase())"
                     class="bg-white rounded-3xl p-7 border-2 border-emerald-900/10 shadow-lg shadow-emerald-900/5 hover:border-gold-500/50 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col justify-between group islamic-card"
                     data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                    
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <div class="w-12 h-12 rounded-2xl bg-rose-50 border border-emerald-200 text-emerald-800 flex items-center justify-center p-3 group-hover:bg-rose-800 group-hover:text-gold-300 group-hover:rotate-6 transition-all duration-300 shadow-sm">
                        ...
                        <div class="pt-3 border-t border-slate-100" x-data="{ isOpen: false }">
                            <div class="text-[11px] font-bold text-emerald-900 mb-2 flex items-center justify-between">
                                ...
                                <button @click="isOpen = !isOpen" class="text-[10px] text-emerald-700 hover:underline font-semibold cursor-pointer" x-text="isOpen ? 'Tutup' : 'Lihat Semua'"></button>
                            </div>
                            
                            <!-- 3 Top Preview -->
                            <ul x-show="!isOpen" class="space-y-1.5 text-xs text-slate-500">
                            ...
                            <!-- Expanded List -->
                            <ul x-show="isOpen" x-transition class="space-y-1.5 text-xs text-slate-500 max-h-48 overflow-y-auto pr-1">
                                @foreach($program->curriculumItems as $curr)
                                    <li class="flex items-center gap-2 truncate py-0.5">
                                        <span class="w-4 h-4 rounded-full bg-rose-100 text-emerald-800 text-[10px] font-bold flex items-center justify-center flex-shrink-0">{{ $loop->iteration }}</span>
                                        <span class="truncate">{{ $curr->title }}</span>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <div class="pt-6 mt-6 border-t border-slate-100 space-y-2">
                        <a href="{{ route('programs.detail', $program->slug) }}" class="block w-full py-2.5 text-center text-xs font-bold rounded-xl bg-rose-50 text-emerald-900 group-hover:bg-rose-800 group-hover:text-gold-300 transition-all border border-emerald-200 shadow-sm">
...
<!-- Interactive Makhraj & Tajwid Sound Simulator -->
<section class="py-20 bg-rose-950 text-white relative border-y-2 border-gold-500/40 overflow-hidden" x-data="{
    selectedMakhraj: 0,
    selectedLetter: null,
    makhrajList: [
        { id: 'al-halq', name: 'Al-Halq (Tenggorokan)', arabic: 'الحَلْق', desc: 'Huruf yang keluar dari tenggorokan bawah (Aqshal Halq), tengah (Wasthul Halq), dan atas (Adnal Halq).', letters: ['ء', 'هـ', 'ع', 'ح', 'غ', 'خ'], example: 'مِنْ خَوْفٍ' },
        { id: 'al-lisan', name: 'Al-Lisan (Lidah)', arabic: 'اللِّسَان', desc: 'Pangkal, tengah, tepi, hingga ujung lidah bertemu langit-langit atau gigi seri.', letters: ['ق', 'ك', 'ج', 'ش', 'ي', 'ض', 'ل', 'ن', 'ر', 'ط', 'د', 'ت', 'ص', 'ز', 'س', 'ظ', 'ذ', 'ث'], example: 'أَنْعَمْتَ' },
        { id: 'as-syafatan', name: 'Asy-Syafatain (Bibir)', arabic: 'الشَّفَتَان', desc: 'Kedua bibir tertutup rapat, terbuka sedikit, atau bibir bawah bertemu ujung gigi seri atas.', letters: ['ف', 'و', 'ب', 'م'], example: 'كُتِبَ' },
        { id: 'al-khaisyum', name: 'Al-Khaisyum (Rongga Hidung)', arabic: 'الخَيْشُوم', desc: 'Pangkal rongga hidung tempat beresonansinya suara dengung (Ghunnah yang sempurna).', letters: ['نّ', 'مّ', 'Ghunnah'], example: 'إِنَّ اللَّهَ' }
    ]
}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center max-w-2xl mx-auto mb-12 space-y-2" data-aos="fade-up">
            ...
            <p class="text-xs sm:text-sm text-emerald-200/80">Klik pada tempat keluarnya huruf untuk melihat huruf hijaiyah dan simulasi pelafalannya.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center" data-aos="zoom-in">
            ...
            <div class="lg:col-span-5 space-y-3">
                <template x-for="(m, idx) in makhrajList" :key="m.id">
                    <button @click="selectedMakhraj = idx; selectedLetter = null" 
                            :class="selectedMakhraj === idx ? 'bg-gradient-to-r from-gold-500 to-amber-600 text-slate-950 font-extrabold shadow-lg scale-[1.02] border-gold-300' : 'bg-rose-900/40 text-emerald-100 border-emerald-800 hover:bg-rose-800/60'"
            ...
            <div class="lg:col-span-7 bg-slate-900/90 border-2 border-gold-500/40 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative">
                ...
                <!-- Letters Grid Badges -->
                <div>
                    <div class="text-xs font-bold text-emerald-400 mb-3 flex items-center justify-between">
                        <span class="flex items-center gap-1.5"><i data-lucide="sparkles" class="w-3.5 h-3.5"></i> Huruf Hijaiyah:</span>
                        <span class="text-[11px] text-slate-400">Klik huruf untuk info makhraj</span>
                    </div>
                    <div class="flex flex-wrap gap-2.5">
                        <template x-for="l in makhrajList[selectedMakhraj].letters" :key="l">
                            <button @click="selectedLetter = l"
                                    :class="selectedLetter === l ? 'bg-gold-500 text-slate-950 scale-110 shadow-lg' : 'bg-rose-950 text-gold-300 hover:bg-gold-500 hover:text-slate-950'"
                                    class="w-11 h-11 rounded-xl border border-gold-500/40 flex items-center justify-center text-xl font-quran shadow-md hover:scale-110 transition-all cursor-pointer select-none"
                                    x-text="l">
                            </button>
                        </template>
                    </div>
                ...
                <!-- Selected Letter Feedback Box -->
                <div x-show="selectedLetter" class="mt-4 p-3 rounded-xl bg-rose-900/40 border border-gold-500/30 flex items-center justify-between text-xs">
                    <div>Huruf Terpilih: <strong class="text-gold-300 font-quran text-lg ml-1" x-text="selectedLetter"></strong></div>
                    <span class="text-emerald-200">Karakter makhraj: <span x-text="makhrajList[selectedMakhraj].name"></span></span>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div class="lg:col-span-6 space-y-6" data-aos="fade-right">
                <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-rose-100 text-emerald-900 text-xs font-bold uppercase tracking-wider">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div class="p-4 rounded-2xl bg-rose-50/50 border border-emerald-100 flex items-start gap-3.5 hover:shadow-md transition-shadow">
                        <div class="w-10 h-10 rounded-xl bg-rose-700 text-gold-300 flex items-center justify-center flex-shrink-0 shadow-sm">
            ...
            <div class="lg:col-span-6 relative" data-aos="fade-left">
                <div class="bg-gradient-to-br from-rose-950 via-rose-900 to-slate-950 text-white rounded-3xl p-8 sm:p-10 shadow-2xl border-2 border-gold-500/40 relative overflow-hidden">
...
<section class="py-24 bg-islamic-subtle">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12" data-aos="fade-up">
            <div>
                <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-rose-100 text-primary-900 text-xs font-bold uppercase tracking-wider">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @foreach($teachers as $teacher)
                <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-emerald-900/10 flex flex-col sm:flex-row items-center sm:items-start gap-6 shadow-sm hover:border-gold-500/40 hover:-translate-y-1 transition-all duration-300 islamic-card" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                    <div class="w-20 h-20 rounded-2xl bg-gradient-to-tr from-rose-900 to-rose-700 text-gold-300 flex items-center justify-center font-bold text-3xl flex-shrink-0 shadow-md border border-gold-400/30">
                        {{ strtoupper(substr($teacher->name, 0, 1)) }}
                    </div>
                    <div class="space-y-2 text-center sm:text-left flex-1">
                        <h3 class="text-lg font-bold text-slate-900 font-serif">{{ $teacher->name }}</h3>
                        <div class="text-xs font-semibold text-emerald-800 bg-rose-50 px-2.5 py-1 rounded-lg inline-block border border-emerald-200">
...
<section class="py-20 bg-gradient-to-r from-rose-950 via-rose-900 to-slate-950 text-white relative overflow-hidden border-t-4 border-gold-500/80">
