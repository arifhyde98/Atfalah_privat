...
<div id="landing-app" v-cloak>
    ...
    <section class="relative overflow-hidden bg-islamic-pattern text-white py-20 lg:py-28 border-b-4 border-gold-500/80">
        ...
        <div class="absolute -top-24 left-1/2 -translate-x-1/2 w-[750px] h-[750px] bg-gradient-to-b from-gold-500/15 via-emerald-500/10 to-transparent rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                ...
                <div class="lg:col-span-7 space-y-6 text-center lg:text-left" data-aos="fade-right">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 pt-4 justify-center lg:justify-start">
                        ...
                        <button @click="scrollToPrograms()" class="w-full sm:w-auto px-8 py-4 rounded-2xl bg-emerald-950/60 hover:bg-emerald-900/80 border border-emerald-500/30 text-white font-semibold text-sm backdrop-blur hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 cursor-pointer">
                ...
                <!-- Right Vue Interactive 4-Pillars Explorer -->
                    ...
                    <div class="bg-gradient-to-b from-emerald-950/95 to-slate-950/95 border-2 border-gold-500/30 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-2xl text-white space-y-5 relative overflow-hidden">
                        ...
                        <div class="flex items-center justify-between border-b border-emerald-800/60 pb-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-[10px] uppercase tracking-widest text-gold-400 font-bold">Alur Pembelajaran Interaktif (Vue 3)</div>
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            <button v-for="p in pillars" :key="p.id"
                                    @click="activePillar = p.id" 
                                ...
                                <span class="text-sm font-arabic font-bold">@{{ p.arabic }}</span>
                                <span class="text-[9px] uppercase tracking-tighter">@{{ p.name }}</span>
                            </button>
                        </div>

                        <!-- Active Pillar Dynamic Card Display with smooth transition -->
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <h4 class="text-sm font-bold text-gold-300 font-serif flex items-center gap-1.5">
                                        <span>@{{ currentPillar.arabic }}.</span>
                                        <span>@{{ currentPillar.name }} — @{{ currentPillar.title }}</span>
                                    </h4>
                                    <span class="text-[10px] px-2 py-0.5 rounded-md bg-white/10 text-emerald-200">@{{ currentPillar.tag }}</span>
                                </div>
                                <p class="text-xs text-emerald-100/90 leading-relaxed">@{{ currentPillar.desc }}</p>
                            </div>
                            <div class="pt-3 flex items-center justify-between text-[11px] text-emerald-300 border-t border-emerald-800/40">
                                <span>Target: @{{ currentPillar.goal }}</span>
                                <span class="text-gold-400 font-semibold flex items-center gap-1">Talaqqi Privat &rarr;</span>
                            </div>
                        </div>
    ...
    <!-- 4 Core Programs Section with Interactive Vue Search & Filter -->
    <section id="programs-section" class="py-24 bg-islamic-subtle relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto mb-10 space-y-3" data-aos="fade-up">
                ...
                <!-- Vue Search & Filter Bar -->
                <div class="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3 max-w-xl mx-auto">
                    <div class="relative w-full">
                        <input type="text" v-model="searchQuery" placeholder="Cari topik modul (e.g., makhraj, tajwid, shalat, huruf)..." class="w-full pl-10 pr-4 py-2.5 rounded-2xl border-2 border-emerald-900/10 focus:border-gold-500 bg-white text-xs shadow-sm outline-none transition-all">
                        <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3.5 top-3"></i>
                    </div>
                    <button v-if="searchQuery" @click="searchQuery = ''" class="px-3 py-2 text-xs font-semibold text-rose-600 hover:underline">
                        Reset Filter
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                <div v-for="(program, index) in filteredPrograms" :key="program.id"
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Interactive Curriculum Preview Toggle in Vue -->
                        <div class="pt-3 border-t border-slate-100">
                            <div class="text-[11px] font-bold text-emerald-900 mb-2 flex items-center justify-between">
                                <span class="flex items-center gap-1"><span class="text-gold-500">✦</span> @{{ program.curriculum_items ? program.curriculum_items.length : 0 }} Modul Silabus:</span>
                                <button @click="toggleExpand(program.id)" class="text-[10px] text-emerald-700 hover:underline font-semibold cursor-pointer">
                                    @{{ isExpanded(program.id) ? 'Tutup' : 'Lihat Semua' }}
                                </button>
                            </div>
                            
                            <!-- 3 Top Preview -->
                            <ul v-if="!isExpanded(program.id)" class="space-y-1.5 text-xs text-slate-500">
                                <li v-for="(curr, cIdx) in (program.curriculum_items || []).slice(0, 3)" :key="cIdx" class="flex items-center gap-2 truncate">
                            ...
                            <!-- Expanded List -->
                            <ul v-else class="space-y-1.5 text-xs text-slate-500 max-h-48 overflow-y-auto pr-1">
                                <li v-for="(curr, cIdx) in (program.curriculum_items || [])" :key="cIdx" class="flex items-center gap-2 truncate py-0.5">
                                    <span class="w-4 h-4 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold flex items-center justify-center flex-shrink-0">@{{ cIdx + 1 }}</span>
                                    <span class="truncate">@{{ curr.title }}</span>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div class="pt-6 mt-6 border-t border-slate-100 space-y-2">
                        <a :href="'/programs/' + program.slug" class="block w-full py-2.5 text-center text-xs font-bold rounded-xl bg-emerald-50 text-emerald-900 group-hover:bg-emerald-800 group-hover:text-gold-300 transition-all border border-emerald-200 shadow-sm">
    ...
    <!-- Vue 3 Interactive Makhraj & Tajwid Sound Simulator -->
    <section class="py-20 bg-emerald-950 text-white relative border-y-2 border-gold-500/40 overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="text-center max-w-2xl mx-auto mb-12 space-y-2" data-aos="fade-up">
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gold-500/20 text-gold-300 text-xs font-bold uppercase tracking-wider border border-gold-500/30">
                    <span>✦</span> Fitur Eksplorasi Tajwid (Vue Interactive) <span>✦</span>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center" data-aos="zoom-in">
                ...
                <div class="lg:col-span-5 space-y-3">
                    <button v-for="(m, idx) in makhrajList" :key="m.id"
                            @click="selectedMakhraj = idx" 
                        ...
                        <div class="flex items-center gap-3">
                            <span class="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center font-bold text-xs">@{{ idx + 1 }}</span>
                            <span class="text-xs sm:text-sm font-semibold">@{{ m.name }}</span>
                        </div>
                        <span class="text-lg font-arabic font-bold">@{{ m.arabic }}</span>
                    </button>
                </div>
                ...
                <div class="lg:col-span-7 bg-slate-900/90 border-2 border-gold-500/40 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
                        <div>
                            ...
                            <h3 class="text-lg sm:text-xl font-bold text-white font-serif">@{{ currentMakhraj.name }}</h3>
                        </div>
                        <span class="text-3xl font-quran text-gold-400">@{{ currentMakhraj.arabic }}</span>
                    </div>

                    <p class="text-xs sm:text-sm text-slate-300 leading-relaxed mb-6">@{{ currentMakhraj.desc }}</p>

                    <!-- Letters Grid Badges with Click to Pronounce Simulation -->
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2.5">
                            <button v-for="l in currentMakhraj.letters" :key="l"
                                    @click="selectLetter(l)"
                                    :class="selectedLetter === l ? 'bg-gold-500 text-slate-950 scale-110 shadow-lg' : 'bg-emerald-950 text-gold-300 hover:bg-gold-500 hover:text-slate-950'"
                    ...
                    <div v-if="selectedLetter" class="mt-4 p-3 rounded-xl bg-emerald-900/40 border border-gold-500/30 flex items-center justify-between text-xs">
                        <div>Huruf Terpilih: <strong class="text-gold-300 font-quran text-lg ml-1">@{{ selectedLetter }}</strong></div>
                        <span class="text-emerald-200">Karakter makhraj: @{{ currentMakhraj.name }}</span>
                    </div>

                    <div class="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
                        <div>Contoh Bacaan: <span class="text-gold-300 font-arabic text-base font-bold ml-1">@{{ currentMakhraj.example }}</span></div>
    ...
    <section class="py-24 bg-islamic-subtle">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div v-for="(teacher, tIdx) in teachers" :key="teacher.id"
                     class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-emerald-900/10 flex flex-col sm:flex-row items-center sm:items-start gap-6 shadow-sm hover:border-gold-500/40 hover:-translate-y-1 transition-all duration-300 islamic-card"
                     data-aos="fade-up" :data-aos-delay="(tIdx + 1) * 100">
                    ...
                    <div class="space-y-2 text-center sm:text-left flex-1">
                        ...
                        <div class="text-xs font-semibold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg inline-block border border-emerald-200">
                            @{{ teacher.teacher_profile ? teacher.teacher_profile.specialization : 'Pengajar Qur\'an Bersanad' }}
                        </div>
                        <p class="text-xs text-slate-600 leading-relaxed">
                            @{{ teacher.teacher_profile ? teacher.teacher_profile.bio : 'Pengajar bersanad mutawatir dengan dedikasi tinggi membimbing perbaikan bacaan dan tadabbur santri.' }}
                        </p>
                    </div>
                </div>
            </div>
...
createApp({
    ...
    computed: {
        ...
        currentMakhraj() {
            return this.makhrajList[this.selectedMakhraj] || this.makhrajList[0];
        },
        filteredPrograms() {
            ...
            return this.programs.filter(p => {
                ...
                const inCurr = (p.curriculum_items || []).some(c => c.title.toLowerCase().includes(q));
                return inName || inTag || inDesc || inCurr;
            });
        }
    },
    methods: {
        getArabicLevel(num) {
            ...
            return 'المستوى ' + (arabicNumbers[num - 1] || num);
        },
        toggleExpand(id) {
            const idx = this.expandedPrograms.indexOf(id);
            if (idx > -1) {
                this.expandedPrograms.splice(idx, 1);
            } else {
                this.expandedPrograms.push(id);
            }
        },
        isExpanded(id) {
            return this.expandedPrograms.includes(id);
        },
        selectLetter(letter) {
